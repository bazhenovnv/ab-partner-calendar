...
CREATE INDEX IF NOT EXISTS "BroadcastDelivery_status_idx" ON "BroadcastDelivery"("status");
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'BroadcastDelivery_subscriberId_fkey'
  ) THEN
    ALTER TABLE "BroadcastDelivery"
      ADD CONSTRAINT "BroadcastDelivery_subscriberId_fkey"
      FOREIGN KEY ("subscriberId") REFERENCES "TelegramSubscriber"("id") ON DELETE CASCADE ON UPDATE CASCADE;
  END IF;
END $$;
